...
window.onload = function () {
    Object.defineProperties(environmentSettings, {
        // размеры поля
        fieldWidth: {
            value: 500,
            writable: false,
    ...
    infoCtx.fillRect(0, 35, environmentSettings.partShipWidth, environmentSettings.partShipHeight);
    infoCtx.strokeStyle = "black";
    infoCtx.font = "italic 10pt Arial";
    infoCtx.strokeText("Промах", 0, 30);
    container.appendChild(infoCanvas);

    var gradient = infoCtx.createRadialGradient(98 + environmentSettings.partShipWidth/2, 35 + environmentSettings.partShipHeight/2, 5, 98 + environmentSettings.partShipWidth/2, 35 + environmentSettings.partShipHeight/2, environmentSettings.partShipWidth/2);
    gradient.addColorStop(0, 'white');
    gradient.addColorStop(1, 'red');
    infoCtx.fillStyle = gradient;
    infoCtx.fillRect(98, 35, environmentSettings.partShipWidth, environmentSettings.partShipHeight);
    infoCtx.strokeStyle = "black";
    infoCtx.font = "italic 10pt Arial";
    ...
    infoCtx.fillRect(198, 35, environmentSettings.partShipWidth, environmentSettings.partShipHeight);
    infoCtx.strokeStyle = "black";
    infoCtx.font = "italic 10pt Arial";
    ...
    infoCtx.fillRect(295, 35, environmentSettings.partShipWidth, environmentSettings.partShipHeight);
    infoCtx.strokeStyle = "black";
    infoCtx.font = "italic 10pt Arial";
...
var gameModule = (function () {
    ...
    // поле для отображения хода игры
    ...
    var gameInfo = {
        // чей ход? 0 - игрок, 1 - компьютер
        gameStep: 0,
        // счетчики потопленных кораблей для каждого игрока
        playerPoints: 0,
        computerPoints: 0,
        startGame: function () {
            this.gameStep = Math.floor(Math.random() * 2);
            this.playerPoints = 0;
    ...
    function initGame(playerName) {
        ...
        function drawFields(fieldContext) {
            for (var x = 0; x <= environmentSettings.fieldWidth; x += environmentSettings.partShipWidth) {
                fieldContext.moveTo(x + 0.5, 0);
                fieldContext.lineTo(x + 0.5, environmentSettings.fieldHeight);
            }
            for (var y = 0; y <= environmentSettings.fieldHeight; y += environmentSettings.partShipHeight) {
                fieldContext.moveTo(0, y + 0.5);
                fieldContext.lineTo(environmentSettings.fieldWidth, y + 0.5);
            }
        ...
        function drawShips(matrix, context) {
            ...
            for (var i = 0; i < 10; i++) {
                for (var j = 0; j < 10; j++) {
                    if (matrix[i][j] === 1) {
                        var x = i * environmentSettings.partShipWidth;
                        var y = j * environmentSettings.partShipHeight;
                        context.fillRect(x, y, environmentSettings.partShipWidth, environmentSettings.partShipHeight);
                    }
                }
            }
        }
        ...
        if (!containerField) {
            ...
            // информация о ходе
            gameInfo.gameStep = Math.floor(Math.random() * 2);
            ...
            canvasPlayerField.width = environmentSettings.fieldWidth + 1;
            canvasPlayerField.height = environmentSettings.fieldHeight + 1;
            ...
            canvasComputerField.width = environmentSettings.fieldWidth + 1;
            canvasComputerField.height = environmentSettings.fieldHeight + 1;
            ...
            generateButton.addEventListener("click", function () {
                gameInfo.startGame();
                if (!gameInfo.gameStep) {
                    containerGameStep.innerText = "Ваш ход";
                } else {
                    containerGameStep.innerText = "Компьютер ходит";
                }
                playerFieldContext.clearRect(0, 0, environmentSettings.fieldWidth, environmentSettings.fieldHeight);
                computerFieldContext.clearRect(0, 0, environmentSettings.fieldWidth, environmentSettings.fieldHeight);
                drawFields(playerFieldContext);
                drawFields(computerFieldContext);
                playerMatrix = generateMatrix();
                computerMatrix = generateMatrix();
                drawShips(playerMatrix, playerFieldContext);
                gameInfo.playerPoints = gameInfo.computerPoints = 0;
                if (gameInfo.gameStep) {
    ...
    function drawMisfire(context, cellX, cellY) {
        var x = cellX * environmentSettings.partShipWidth;
        var y = cellY * environmentSettings.partShipHeight;
    ...
    function drawHit(context, cellX, cellY) {
        var x = cellX * environmentSettings.partShipWidth;
        var y = cellY * environmentSettings.partShipHeight;
        var gradient = context.createRadialGradient(x + environmentSettings.partShipWidth/2, y + environmentSettings.partShipHeight/2, 5, x + environmentSettings.partShipWidth/2, y+environmentSettings.partShipHeight/2, environmentSettings.partShipWidth/2);
        gradient.addColorStop(0, 'white');
        gradient.addColorStop(1, 'red');
    ...
    function computerFieldOnClick(e) {
        ...
        function checkCursorPosition(e) {
            ...
            cellX = Math.min(cellX, environmentSettings.fieldWidth);
            cellY = Math.min(cellY, environmentSettings.fieldHeight);
            cellX = Math.floor(cellX/environmentSettings.partShipWidth);
            cellY = Math.floor(cellY/environmentSettings.partShipHeight);
        }
    }
    ...
    // конец игры
    function isGameOver() {
        if (gameInfo.playerPoints === 10) {
            containerGameStep.innerText = "Игра окончена. Победил " + name;
            return true;
        } else if (gameInfo.computerPoints === 10) {
